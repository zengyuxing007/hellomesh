...
// CtlAPIConf relates to hellomesh control api.
...
// HelloMeshConf is configurations for conductor server.
type HelloMeshConf struct {
...
// NewHelloMeshConfFile parses configuration file.
func NewHelloMeshConfFile(confPath string) (*HelloMeshConf, error) {
	buf, err := ioutil.ReadFile(confPath)
	if err != nil {
		return nil, fmt.Errorf("could not read config - %s", err)
	}

	return NewHelloMeshConfYaml(buf)
}

// NewHelloMeshConfYaml parses configuration YAML string.
func NewHelloMeshConfYaml(yamlbytes []byte) (*HelloMeshConf, error) {
	conf := &HelloMeshConf{}
